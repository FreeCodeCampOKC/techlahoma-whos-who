...
function check() {
    ...
    if (
        firstNameSelected === person.firstName &&
        lastNameSelected === person.lastName
    ) {
        ...
        resultElement.innerHTML =
            halfRightMessages[
                Math.floor(Math.random() * halfRightMessages.length)
            ];
...
function updateScore() {
    document.getElementById("done").innerHTML = people.filter(
        (x) => x.done
    ).length;
    document.getElementById("left").innerHTML = people.filter(
        (x) => x.picture && !x.done
    ).length;
}
